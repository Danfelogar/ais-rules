...
You are helping to spin up a new feature spec for this application, form a short idea provided in the user input below. Always adhere to any rules or requirements set out in any CLAUDE.md files when responding
...
- A human friendly feature title in kebab-case(e.g. new-heist-from)
- A safe git branch name not already taken(e.g. claude/feature/new-heist-form)
- A detailed markdown sepc file under the _specs/ directory
...
Before making any content, switch to a new Git branch using the `branch-name` derived from the `$ARGUMENTS`. If the branch name is already taken, then append a version number to it: e.g. `claude/feature/card-component-01`
